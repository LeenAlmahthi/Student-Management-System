...
let selectdelete = -1;
const matrial = [
  {name: 'Java script', Price: 30, StatusCourse: 'Enroll'},
  {name: 'C++', Price: 40, StatusCourse: 'Enroll'},
  {name: 'Database', Price: 70, StatusCourse: 'Enroll'},
];
// Create cards Course
...
function BehaverAddBtn() {
  ...
  del_btn.type = 'button';
  del_btn.textContent = ('-');
  del_btn.classList.add('delete_btn_add');
  form_add.append(del_btn);
  del_btn.addEventListener( 'click' ,  (event)  =>  {
    event.preventDefault();
    hold_form.style.display = "none";
    console.log('click button  in Add');
})
  ...
  form_add.addEventListener('submit', (event) => {
    try {
      ...
      $.ajax({
        url: 'https://localhost:54888/AvaliableCourse',
        method: 'GET',
        success: function(response) {
          console.log('Data Success sending request:');

          let des_course =
              response.find(des_course => des_course.name === name_input.value);
          if (!des_course) {
          ...
          $.ajax({
            ...
            error: function(error) {
              console.log(
                  'somethings wrongs happens with add course :(', error);
            }
          });
        },
        error: function(error) {
...
function BehaverDelete() {
  // alert("The course has been deleted successfully"); // how to show a toest
  // message in Js matrial.splice(selectdelete, 1);
  ...
  $.ajax({
    ...
    success: function(response) {
      console.log('Data Success sending request:');
      let des_course =
          response.find(des_course => des_course.name === name_course);
      if (!des_course) {
        console.log('This course does not have a section!');
        // let btn = document.querySelector("#hold_form");
        // btn.style.display = "none";
      ...
      const del_url = 'https://localhost:54888/Course?id=' + des_course.id;
      $.ajax({
        url: del_url,
        method: 'Delete',
        success: function(response) {
