function validation() {
  try {
    ...
    if (!/^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*]).+$/.test(Password))
      throw (
          'password must have atleast 1 captial char and numebr and one spicial chat ');
    if (Password !== confirm_password) throw ('password must be match');
...
function CallApi() {
  ...
  try {
    ...
    $.ajax({
        url: 'https://localhost:54888/api/authApplication/Register', 
        method: 'POST',
...
const data = {username: $('#name').val(), Password: $('#Password').val()};
data_body = JSON.stringify(data);
$.ajax({
    url:  'https://localhost:54888/api/authApplication/Login',
    method: 'POST',
    contentType: 'application/json',
    data: data_body,
    success: function(data) {
